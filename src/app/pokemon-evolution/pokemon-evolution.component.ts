...
export class PokemonEvolutionComponent implements OnInit
{
  ...
  ngOnInit(): void
  {
    this.detailsObs.subscribe((det) =>
    {
      ...
      this.speciesObs.subscribe((spec) =>
      {
        ...
        {
          this.evolution = evo;
          this._pokemonApiService.getPokemonDataFromUrl<IPokemonSpecies>(evo.chain.species.url).subscribe(spec =>
            {
              ...
              this.evolvedSpeciesObs = this._pokemonApiService.getPokemonDataFromUrl<IPokemonSpecies>(nextSpeciesUrl);
  ...
  private _getEvolutionChain(evo: IPokemonEvolution) : {name: string, url: string, level: number}[]
  {
    ...
    while(chainLink != null)
    {
      ...
      if(!species)
        break;
      chain.push({ name: species.name, url: species.url, level: chainLink.evolution_details[0]?.min_level });
  ...
  {
    let nextIndex = this._evolutionChain.findIndex(x => x.name === this.details.name) + 1;
    return nextIndex < this._evolutionChain.length ? this._evolutionChain[nextIndex] : undefined;
  }
